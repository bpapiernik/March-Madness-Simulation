...
away_rows <- temp[which(temp$team_home_away == "away"), c(1:2,4,6,9:11,18,27:31,37)]
home_rows <- temp[which(temp$team_home_away == "home"), c(1:2,4,6,9:11,18,27:31,37)]

names(away_rows)[c(4:14)] <- c("away_id", "away_team", "away_mascot", "away_abbreviation", "away_score",
                               "a_flagrant_fouls", "a_total_fouls", "a_free_throw_pct",
                               "a_free_throws_made", "a_free_throws_attempted", "a_technical_fouls")
names(home_rows)[c(4:14)] <- c("home_id", "home_team", "home_mascot", "home_abbreviation", "home_score",
                               "h_flagrant_fouls", "h_total_fouls", "h_free_throw_pct",
                               "h_free_throws_made", "h_free_throws_attempted", "h_technical_fouls")

cbb_season_results <- cbind(home_rows, away_rows)

cbb_season_results <- cbb_season_results[,-c(15,16,17)]
...
final_elo <- rbind(final_elo,temp_elo)
```
...
temp_elo$season <- 2021 # change for each year
final_elo <- rbind(final_elo,temp_elo)
```
...
temp_elo$season <- 2019 # change for each year
final_elo <- rbind(final_elo,temp_elo)
```
...
temp_elo$season <- 2018 # change for each year
final_elo <- rbind(final_elo,temp_elo)
```
...
temp_elo$season <- 2017 # change for each year
final_elo <- rbind(final_elo,temp_elo)
```
...
temp_elo$season <- 2016 # change for each year
final_elo <- rbind(final_elo,temp_elo)
```
...
temp_elo$season <- 2015 # change for each year
final_elo <- rbind(final_elo,temp_elo)
```
...
temp_elo$season <- 2014 # change for each year
final_elo <- rbind(final_elo,temp_elo)
```
...
temp_elo$season <- 2013 # change for each year
final_elo <- rbind(final_elo,temp_elo)
```

```{r}
write.csv(final_elo, "final_elo.csv", row.names = FALSE)
```
...
write.csv(total_cbb_season_results, "historic_regular_season1.csv", row.names = FALSE)
```
